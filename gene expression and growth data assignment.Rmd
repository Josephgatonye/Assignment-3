...
```
## Question 5 
The first step is to create a png file using the png() command and add the histogram plot created with the hist () command. The second step is to import the file using the ![]() command into the report.
step 1 :saving histogram as png file
```{r,chunk5,echo=TRUE}
png("gene_expression.png") 
hist(x$ColumnMeans,main = "Histogram of mean values",xlab ="column means")
dev.off()
```
step 2 :pasting into report
![](gene_expression.png "gene_expression Plot") 
